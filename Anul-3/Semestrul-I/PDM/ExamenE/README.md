...
DA 1. Primul ecran prezinta informatii despre task-uri, cu operatii disponibile atat in modul
...
DA Utilizatorul poate face click pe un tag pentru a vedea task-urile marcate cu acel tag.
...
DA Utilizatorul poate face click pe un task din lista pentru a deschide al doilea ecran
...
DA In timpul efectuarii operatiei PUT pentru un task, lista de task-uri din primul ecran
prezinta textul 'Sending...' in dreptul acelui task.

DA Operatia PUT poate esua (409 Version conflict) daca altcineva a modificat intre timp
...
DA Daca exista task-uri modificate local si netrimise la server, utilizatorul poate relua
trimiterea task-ului facand click pe 'Not sent'.

DA Daca exista task-uri modificate local pentru care trimiterea a esuat din cauza unui conflict,
...
DA Serverul emite notificari prin ws pe localhost:3000. O notificare contine un task modificat
...
DA In timpul descarcarii listei de taskuri de pe server (1) sau a descarcarii unui task (7),
