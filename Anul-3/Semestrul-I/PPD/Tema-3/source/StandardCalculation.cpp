...
void StandardCalculation::calculator(int rank) {
    if (rank == 0) {
        ...
        for (int pid = 1; pid < P; pid++) {
            ...
            MPI_Ssend(firstNumber, batchSize,MPI_INT, pid, 0,MPI_COMM_WORLD);
            MPI_Ssend(secondNumber, batchSize,MPI_INT, pid, 0,MPI_COMM_WORLD);

            int *result = new int[batchSize];
            MPI_Recv(result, batchSize,MPI_INT, pid, 0,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
            for (int i = 0; i < batchSize; i++) {
                out << result[i] << " ";
            }
            if (pid == P - 1) {
                int flag;
                MPI_Recv(&flag, 1, MPI_INT, pid, 0,MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                if (flag != 0) {
                    out << flag;
                }
            }
        ...
        const int dimension = N_Max / (P - 1);
        int extra = N_Max % (P - 1);
        int batchSize = dimension + ((rank - 1) < extra);
        ...
        MPI_Recv(secondNumber, batchSize,MPI_INT, 0, 0,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
        ...
        for (int i = 0; i < P-2; i++) {
            ...
            passCarry(result, batchSize, carry);
        }

        MPI_Send(result, batchSize,MPI_INT, 0, 0,MPI_COMM_WORLD);
        if (rank == P - 1) {
