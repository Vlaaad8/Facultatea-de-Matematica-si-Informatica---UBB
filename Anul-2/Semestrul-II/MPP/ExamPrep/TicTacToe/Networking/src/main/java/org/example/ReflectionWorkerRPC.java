...
public class ReflectionWorkerRPC implements Runnable, IObserver {
    ...
    public void run() {
        while (connected) {
            try {
                ...
                logger.error("Error in worker run: {}", e.getMessage());
                connected = false;
            }
            try {
                Thread.sleep(1000);
    ...
    private Response handleLOGIN(Request request) {;
        ...
        try {
            logger.trace("Angajatul pe care l-am primit are datele {}", employee.getName());
    ...
    private Response handleSAVE_CONFIG(Request request){
        ConfigurationDTO gameDTO =(ConfigurationDTO) request.data();
        Configuration game= DTOUtils.getFromDTO(gameDTO);
        logger.info("Am primit {} {}",game.getConfig(),game.getId());
        try{
            server.saveConfig(game);
            return (new Response.Builder()).type(ResponseType.OK).build();
        }
        catch(Exception e){
            throw new RuntimeException("Eroare la get configuration game!");
