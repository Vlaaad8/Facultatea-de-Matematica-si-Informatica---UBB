...
public class UserDbRepository extends AbstractDbRepository<Long, User> {
    public UserDbRepository(String url, String password, String username, Validation<User> validator) {
    ...
    public Optional<User> save(User entity) {
        int rez = -1;
        try (Connection connection = DriverManager.getConnection(getUrl(), getUsername(), getPassword());
             PreparedStatement statement = connection.prepareStatement("INSERT INTO \"User\" (firstname, lastname) VALUES (?, ?)")) {
    ...
    public Optional<User> update(User entity) {
        try (Connection connection = DriverManager.getConnection(getUrl(), getUsername(),getPassword());) {
            ...
            getValidator().validate(entity);
            PreparedStatement statement = connection.prepareStatement("UPDATE \"User\" SET firstname=?, lastname=? WHERE id_user = ?");
            statement.setString(1, entity.getFirstName());
            statement.setString(2, entity.getLastName());
