...
public class MainMenuView implements Observer<FriendshipEntityChange>{
    ...
    public void handlePage(MouseEvent mouseEvent) throws IOException {
        FXMLLoader loader = new FXMLLoader();
        loader.setLocation(getClass().getResource("../user-page.fxml"));
        AnchorPane root = (AnchorPane) loader.load();
        Scene scene = new Scene(root);
        Stage stage2 = new Stage();
        stage2.setScene(scene);
        stage2.setTitle("Yahoo Messenger");
        UserPage userPageView = loader.getController();
        userPageView.setService(userService,friendshipService,requestService,IDUser);
        stage2.show();
    }
}
