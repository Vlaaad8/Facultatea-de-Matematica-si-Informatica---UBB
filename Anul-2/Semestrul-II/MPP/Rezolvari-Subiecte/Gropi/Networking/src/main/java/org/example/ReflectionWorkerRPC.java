...
public class ReflectionWorkerRPC implements Runnable, IObserver {
    ...
    private Response handleGET_CONFIG(Request request){
        ListDTO list= (ListDTO) request.data();
        try{
            System.out.println(list.getColumn()+" "+list.getConfig());
            Configuration configuration= server.getRandomConfig(list.getColumn(),list.getConfig());
    ...
    private Response handleGET_FINISHED(Request request){
        try{
            String name= (String) request.data();
            List<Game> games=server.getFinishedGamesByName(name);
            List<GameDTO> games1=new ArrayList<>();
            for(Game game : games){
                games1.add(DTOUtils.getDTO(game));
            }
            ...
            throw new RuntimeException("Eroare la get finished game!");
        }
    }
