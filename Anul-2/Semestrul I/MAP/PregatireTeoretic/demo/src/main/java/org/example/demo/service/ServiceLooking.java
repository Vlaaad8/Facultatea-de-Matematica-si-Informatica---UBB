...
public class ServiceLooking implements Observable<LookingEntityChange> {
    ...
    public Long getLastID(int window){
        Long max=-1L;
        try {
            for(Looking look: repositoryLooking.findAll()){
                if(look.getId()>max && window==look.getId()){
                    max=look.getId();
                }
            }
        } catch (SQLException e) {
    ...
    public Long getLookingByDestinations(String to,String from) throws SQLException {
        for(Looking look:repositoryLooking.findAll()){
            if(look.getDeparture().equals(to) && look.getDestination().equals(from)){
                return look.getId();
            }
        }
        return 0L;
    ...
    public int numberViewers(String from,String to) throws SQLException {
        int nr=0;
