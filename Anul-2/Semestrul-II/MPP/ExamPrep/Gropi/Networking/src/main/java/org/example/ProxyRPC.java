...
public class ProxyRPC implements IServices{
    ...
    @Override
    public Configuration getRandomConfig(int column,int configurationID) {
        ListDTO list=new ListDTO(column,configurationID);
        Request request= new Request.Builder().type(RequestType.GET_CONFIG).data(list).build();
        this.sendRequest(request);
        try{
            Response response = this.readResponse();
            if (response.type() == ResponseType.OK) {
    ...
    public List<Game> getFinishedGamesByName(String name) {
        Request request=new Request.Builder().type(RequestType.GET_FINISHED).data(name).build();
        this.sendRequest(request);
        try{
            Response response=this.readResponse();
            if(response.type() == ResponseType.OK){
